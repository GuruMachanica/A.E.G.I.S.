...
Clean PyTorch Model Architecture definition.
"""
import random
...
class GraphAttentionLayer(nn.Module):
    ...
    def _apply_BN(self, x: Tensor) -> Tensor:
        org_size = x.size()
        x = x.contiguous().view(-1, org_size[-1])
        x = self.bn(x)
...
class HtrgGraphAttentionLayer(nn.Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor, master: Tensor = None):
        num_type1 = x1.size(1)
        num_type2 = x2.size(1)
        ...
        x1_out = x.narrow(1, 0, num_type1)
        x2_out = x.narrow(1, num_type1, num_type2)
...
class AASISTModel(nn.Module):
    ...
    def forward(self, x: Tensor, Freq_aug: bool = False):
        ...
        out_T_aug2, out_S_aug2, master_aug2 = self.HtrgGAT_layer_ST22(out_T2, out_S2, master=master2)
        out_T2 = out_T2 + out_T_aug2
        out_S2 = out_S2 + out_S_aug2
        master2 = master2 + master_aug2
