...
class AuthNotifier extends Notifier<AuthState> {
  ...
  Future<bool> loginLocalGuest({String? name, String? phone}) async {
    state = state.copyWith(
      isAuthenticated: true,
      isLoading: false,
      accessToken: 'LOCAL_EDGE_OFFLINE_TOKEN',
      userId: 'local_shield_operator',
      fullName: name ?? 'Shield Commander',
      phoneNumber: phone ?? '',
      email: 'local.guardian@aegis.shield',
      clearError: true,
    );
    await _persist();
    await ref.read(localDatabaseProvider).logEvent('auth', 'Local operator entered shield');
    return true;
  }
